...
async function buildCandidateVM(db, candidateId) {
  ...
  const appSql = `
    SELECT a.${APP_PK} AS application_id,
         ...
         a.resume_url,
         a.cover_letter_url,
         COALESCE(a.expected_salary_range, to_jsonb(a)->>'expected_salary_range') AS expected_salary_range,
         COALESCE(a.years_experience::text, to_jsonb(a)->>'years_experience') AS years_experience,
         COALESCE(a.application_source, to_jsonb(a)->>'application_source') AS application_source,
         COALESCE(a.photo_url, to_jsonb(a)->>'photo_url', to_jsonb(a)->>'photo') AS photo_url,
