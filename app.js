...
// ... (Environment validation skipped for brevity, keeping existing code) ...
...
const {
  // ... existing config ...
  PUBLIC_APP_ALLOWED_ORIGIN = process.env.PUBLIC_APP_ALLOWED_ORIGIN,
} = process.env;

// Allow multiple origins (comma separated or specific)
const allowedOrigins = (PUBLIC_APP_ALLOWED_ORIGIN || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
const corsOptions = {
  origin: function (origin, callback) {
    if (!origin) return callback(null, true); // Allow mobile/curl (no origin)
    if (allowedOrigins.includes(origin) || allowedOrigins.includes("*")) {
      return callback(null, true);
    }
    // Allow Electron file:// origins or specific localhosts if needed for dev
    if (
  ...
  credentials: true, // Required for cookies (session) to flow
...
// APPLY CORS globally (must be before routes/auth)
app.use(cors(corsOptions));
app.options("*", cors(corsOptions)); // Preflight checks
...
// Generate a temporary signed URL for a file (for resume/cover letter view)
// Must be BEFORE /files/* wildcard
app.get("/files/sign", ensureAuthenticated, (req, res) => {
  const url = req.query.url;
  const ttl = parseInt(req.query.ttl || "60", 10); // default 60 seconds
  if (!url) return res.status(400).json({ error: "url_required" });

  // Extract key from URL (remove origin and base path)
  // Assumes URL structure: .../api/files/<key>
  // or just <key> if passed directly
  let key = url;
  ...
  const sig = signParams(key, exp);

  if (!sig) return res.status(500).json({ error: "signing_failed" });

  const signedUrl = `${FILES_PUBLIC_URL}/files-signed?key=${encodeURIComponent(
    key
  )}&exp=${exp}&sig=${sig}${req.query.download === "1" ? "&download=1" : ""}`;
  res.json({ signedUrl });
});
...
// --- Helper Functions for File Signing ---

function signParams(key, exp) {
  if (!FILES_SIGNING_SECRET) return null;
  const data = `${key}:${exp}`;
  return crypto
...
function verifySignedParams(key, exp, sig) {
  if (!FILES_SIGNING_SECRET || !sig || !exp) return false;
  // Check expiration
  if (Date.now() > parseInt(exp, 10)) return false;
  // Re-sign and compare
  const expected = signParams(key, exp);
